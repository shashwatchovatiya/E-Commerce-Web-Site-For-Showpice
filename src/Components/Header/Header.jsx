...
export default function Header() {
  ...
  const handleSearchSubmit = (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      // Trigger search or navigation logic here if needed
    }
  };
  ...
  const handleSearchChange = useCallback(
    debounce((e) => {
      ...
      // Mocked search logic - replace with your actual search logic
      const mockResults = [
        {
          id: 1001,
          img: img1001,
      ...
      if (value === "") {
        ...
        // Logic to filter results based on the search input
        // setFilteredResults(filteredData);
        setShowResults(true); // Show results only if searchInput is not empty
      }
    }, 300),
    [setSearchInput, setFilteredResults, setShowResults]
  );

  return (
    <>
      <header
        className={`container header flex justify-around items-center transition-all w-full py-5 bg-[#F9F1E7] ${
          isMenuOpen
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center w-full sm:gap-4 gap-10">
            <div className="relative group sm-block">
              <input
                ...
                className="w-[250px] sm:w-[300px] group-hover:w-[250px] md:group-hover:w-[300px] transition-all duration-300 rounded-2xl border border-gray-300 px-2 py-1 focus:outline-none focus:border-1 focus:border-primary"
              />
      ...
      {showResults && filteredResults.length > 0 && (
        <div className="grid my-10 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-6">
